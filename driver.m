...
%    end
   ...
   for ll_h = 1 : n_int_h
       ...
       dx_dxi_bottom = 0.0;
       for aa = 1 : 2
           dx_dxi_bottom = dx_dxi_bottom + x_ele(aa) * Quad(aa,xi(ll_h),-1); % -1 makes the zero in Quad
       end
       for aa = 1 : 2
           Nah(aa) = Nah(aa) + weight(ll_h) * Quad(aa,xi(ll_h),-1) * h_ele(aa) * dx_dxi_bottom;
       end
       
       % Top boundary of one element
       dx_dxi_top = 0.0;
       for aa = 3 : 4
           dx_dxi_top = dx_dxi_top + x_ele(aa) * Quad(aa,xi(ll_h),1); % +1 makes the zero in Quad
       end
       for aa = 3 : 4
           Nah(aa) = Nah(aa) + weight(ll_h) * Quad(aa,xi(ll_h),1) * h_ele(aa) * dx_dxi_top;
       end
