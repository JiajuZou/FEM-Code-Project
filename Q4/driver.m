...
for n = 1 : NN
    ...
    for nx = 1 : n_np_x
        if n <= 1 / dt                            % 1 / dt 
            g_b((n_np_y-1) * n_np_x + nx) = t(n); % t(1/dt+1) = 1; t(1) = 0;
        else
            g_b((n_np_y-1) * n_np_x + nx) = 1;
        end
    end
    
    % Construct one matrix to store the g_dt data
    g_dt_b = zeros(n_np,1);
    % The top g_dt data = 1 only when t < 1
    for nx = 1 : n_np_x
        if n <= 1 / dt
            g_dt_b((n_np_y-1) * n_np_x + nx) = 1;
        end
    end
    ...
    for ee = 1 : n_el
        ...
        g_ele    = g_b(IEN(1:n_en,ee));        % g boundary data of each element
        g_ele_dt = g_dt_b(IEN(1:n_en,ee));     % g_dt boundary data of each element
            ...
            if PP > 0
                ...
                for bb = 1 : n_en
                    ...
                    if QQ > 0
                        ...
                        F(PP) = F(PP) - (k_ele(aa,bb) * g_ele(bb) + m_ele(aa,bb) * g_ele_dt(bb));
                        % m_ele * g_ele_dt should be added here
                        % g_ele_dt = 1 if t < 1 since g = t
...
%     lambda = eigs( inv(M) * K, 1 );
    ...
    for ii = 1 : n_np
        ...
        if index > 0
            ...
            disp(ii) = g_b(ii);   % the g-data boundary where ID = 0
        end
    end
